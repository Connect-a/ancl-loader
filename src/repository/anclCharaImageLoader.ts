...
const mergeImg = async (dir: JSZip | null | undefined, canvas: HTMLCanvasElement, blobMap: Map<string, Blob>) => {
  ...
  for (const mergePattern of imageMergePatterns as Array<Array<string>>) {
    ...
    const num = mergedPrefixes.filter(x => x === prefix).length.toString().padStart(2, "0");
    const merged = (await mergePng(canvas, mergePattern.map(x => blobMap.get(x) ?? new Blob()))) ?? new Blob();
